...
class ErrorPlotter(Plotter):
    ...
    def plot_scatter_with_lines_and_error_bars(self, log=False, data_labels_list=None, x_axis_labels=None, **kwargs):
        plt = self.plt
        x_data = self.data[0]
        ...
        for i in range(int(self.num_data/2)):
            if data_labels_list is not None:
                plt.scatter(x_data, self.data[2 * i + 1], color=colors[i], ls='-', label=data_labels_list[i])
            else:
                plt.scatter(x_data, self.data[2*i+1], color=colors[i], ls='-')
            plt.errorbar(x_data, self.data[2*i+1], yerr=self.data[2*(i+1)], color=colors[i])

        # change x-axis to custom
        if x_axis_labels is not None:
            assert len(x_data) == len(x_axis_labels), f'Number of ticks (= {len(x_data)}) should be the same as ' \
