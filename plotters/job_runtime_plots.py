...
class JobRuntimePlotter(Plotter):
    ...
    def plot_cores_vs_SU_per_SCF_per_atom(self, marker_size=10, filter_heading=None, filter_string_for_plotting=None, **kwargs):
        plt = self.plt
        ...
        SU_per_SCF_per_atom_from_elapsed_time = [
            round((self.elapsed_times[i]*self.n_procs[i])/(n_atoms[i] * self.n_scf_iters[i]), 10)
        ...
        assert len(self.n_procs) == len(self.n_atoms) == len(self.n_scf_iters) == len(self.elapsed_times) \
            ...
            try:
                ...
                for i in range(len(self.n_procs)):
                    if filter_string_for_plotting in unique_filter_keys:
                        ...
                        raise ValueError(f'String to selected for filtering data, {filter_string_for_plotting}, '
                                         f'is invalid.\n Available strings are {unique_filter_keys} ')
            except ValueError:
                raise ValueError(f'{filter_heading} is not in the list of headings: {self.labels} \n '
