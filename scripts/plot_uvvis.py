...
@click.option('-o', '--os-filter', type=float, default=0.05,
...
@click.option('-yl', '--ylabel', default="Molar absorptivity / L mol$^{-1}$ cm$^{-1}$", help='Label for y-axis.')
...
def entry_point(filename, save_folder, write_filename, os_filter,  xmin, xmax, ymin, ymax, xlabel, ylabel, title,
    ...
    plotter = UVVisPlotter(
        ...
        save_format=save_format, os_filter=os_filter
    )
    plotter.plot_wavelengths(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, xlabel=xlabel, ylabel=ylabel, title=title,
