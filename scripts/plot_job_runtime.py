...
@click.option('-m', '--marker_size', default=10, type=int, help='size of marker in scatter plot')
@click.option('-h', '--heading', default=None, type=str, help='Heading string to apply filter on.')
@click.option('-p', '--plot-filter', default=None, type=str, help='Filter string for plotting.')
...
def entry_point(filename, save_folder, write_filename, marker_size, heading, plot_filter, x_col, y_col,
                xmin, xmax, ymin, ymax, xlabel, ylabel, title, label_fontsize, save_format):
    """Example usage:\n
    ...
    plotter.plot_cores_vs_SU_per_SCF_per_atom(
        marker_size=marker_size, filter_heading=heading, filter_string_for_plotting=plot_filter,
        xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, xlabel=xlabel, ylabel=ylabel, title=title)
