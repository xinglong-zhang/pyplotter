...
@click.option('-o', '--octants', type=bool, default=True, help='Plot axes planes to split 3D space into octants.')
...
@click.option('-sc', '--scale-factor', default=0.4, help='Scale factor for determining ranges on axes.')
...
def entry_point(filename, save_folder, write_filename, plot_mode, x_col, y_col, z_col, bar_width,
                xmin, xmax, ymin, ymax, zmin, zmax, lines, octants, xlabel, ylabel, zlabel, scale_factor,
                title, plot_width, plot_height, label_fontsize, save_format):
    create_logger()
    plotter = Plotter(
    ...
    plotter.plot_3d_line_scatter_bars(
        plot_mode=plot_mode, x_col=x_col, y_col=y_col, z_col=z_col, bar_width=bar_width,
        xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, zmin=zmin, zmax=zmax, lines=lines, octants=octants,
        xlabel=xlabel, ylabel=ylabel, zlabel=zlabel, scale_factor=scale_factor, title=title
    )
